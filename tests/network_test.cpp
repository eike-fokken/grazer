...
//   auto node4_ptr = std::make_shared<Network::Node>(node4); //mache es zum
//   shared_ptr auto node3_ptr = std::make_shared<Network::Node>(node3); //mache
//   es zum shared_ptr Network::Edge edge1(node4_ptr, node3_ptr); //gerichtete
//   Kante: 4->3 auto edge1_ptr = std::make_shared<Network::Edge>(edge1);
//   //mache es zum shared_ptr
...
//   std::cout << "this edge exists: " << b << std::endl; //? klappt nicht so
//   ganz
...
//   /*std::vector<std::weak_ptr<Network::Edge>> g1 = node4_ptr->starting_edges;
//   //iwas stimmt hier nicht

//   for (auto it = g1.begin(); it != g1.end(); it++){
//       //it is ein pointer vom Typ Network::Edge, mit .lock() machen wir
//       daraus shared_ptr und können auf diesen zugreifen int s = (*it).lock()
//       -> get_starting_node() -> get_id(); std::cout << "( " << s; int e =
//       (*it).lock() -> get_ending_node() -> get_id(); std::cout << " )" << e;
//   }*/
// }
