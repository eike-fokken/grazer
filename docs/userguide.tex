...
\section{Usage}
\label{sec:usage}
Grazer can be invoked by supplying a directory path leading to the input files:
\begin{lstlisting}[language=bash,firstnumber=1]
$ grazer run path/to/problemdirectory
\end{lstlisting}
problemdirectory itself should contain a directory called \emph{problem},
which contains a \emph{problem\sco data.json} file.
Then grazer will try to compute a solution to the problem defined in
\emph{problem\sco data.json}
and either save the computed solution to a folder \emph{output} in \emph{problemdirectory}
or supply an error message of why a solution couldn't be computed.
...
The top-level structure of \emph{problem\sco data.json} is given in \autoref{fig:problem_data.json}.
\begin{figure}[ht]
  \centering
