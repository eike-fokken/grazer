...
public:
  ...
  /// It gives the new node simply an id equal to the number of the previosly exiting nodes in the net.
  ...
  /// This function removes a node from the net by id by going through a vector of exiting nodes.
  ...
  /// The order of nodes in the edge is not relevant. Its reverse edge is thus also checked.
  /// @param node_id_1 Starting Node of edge
  /// @param node_id_2 Ending Node of edge
  /// @returns Return True if the given edge or its reverse edge exists and false if neither exist.
  bool exists_edge_between(int node_id_1, int node_id_2) const;


  /// This function returns (by providing the nodes id) a pointer of type <Network::Node>.
  /// @param id Nodes id
