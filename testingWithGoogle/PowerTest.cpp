...
TEST(testPower, test_P_and_Q_1) {
  ...
  auto b = n2.set_indices(a);

  Eigen::VectorXd rootvalues(b);
  double last_time = 0.0;
  double new_time = 0.0;
  Eigen::VectorXd last_state(b);
  Eigen::VectorXd new_state(b);
  new_state << V1, phi1, P1,Q1,V2, phi2,P2,Q2;
  ...
  EXPECT_DOUBLE_EQ(rootvalues[0], -new_state[0] + n1.P(new_state));
  EXPECT_DOUBLE_EQ(rootvalues[1], -new_state[1] + n1.Q(new_state));
  
  EXPECT_DOUBLE_EQ(rootvalues[2], new_state[2] - V1_bd);
  EXPECT_DOUBLE_EQ(rootvalues[3], new_state[3] - phi1_bd);


  EXPECT_DOUBLE_EQ(rootvalues[4], -new_state[4] + n2.P(new_state));
  EXPECT_DOUBLE_EQ(rootvalues[5], -new_state[5] + n2.Q(new_state));
  
  EXPECT_DOUBLE_EQ(rootvalues[6], new_state[4] - P2_bd);
  EXPECT_DOUBLE_EQ(rootvalues[7], new_state[5] - Q2_bd);
