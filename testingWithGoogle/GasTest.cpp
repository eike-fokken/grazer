...
namespace Eigen {
  ...
  void to_json(nlohmann::json& j, const MatrixBase<Derived>& eig) {
    ...
    //               "This function is only for vectors, not for matrices.");
...
nlohmann::json make_value_json(std::string id, Eigen::MatrixBase<Derived0> const & condition0, Eigen::MatrixBase<Derived1> const &condition1){
...
public:
  ...
  TEST_F(GasTEST, evaluate_and_evaluate_state_derivative) {
    ...
    auto b1 = make_value_json(node1_topology["id"], flow1start,
                                           flow1end);
    ...
    Eigen::Vector2d cond0(pressure_start,pressure_end);
    Eigen::Vector2d cond1(flow_start, flow_end);

    auto sp0_initial = make_value_json(shortpipe_topology["id"],cond0,cond1);
  ...
  //   //                              new_state);
  //   // sp0.evaluate_state_derivative(&handler, last_time, new_time,
  //   // last_state,new_state); sp1.evaluate_state_derivative(&handler, last_time,
  ...
  //   //   std::cout <<  "\033[1;31mMaybe also test evaluate in this multi-edge
  //   //   setting.\033[0m\n" <<std::endl;
  // }
  ...
  //   //   ((rootvalues1-rootvalues)/epsilon).segment<2>(1); Eigen::VectorXd diff2
  //   //   = ((rootvalues2-rootvalues)/epsilon).segment<2>(1); Eigen::VectorXd
  ...
  //   //       -0.5*Delta_t*bl.dsource_dstate(new_left); expected_J.block<2, 2>(0,
  //   //       2) =
  ...
  //   //                    {"type", "Vphi"},
  //   //                    {"data", json::array({{{"time", 0.},
  //   //                                           {"values", json::array({V1_bd,
  //   //                                           phi1_bd})}}})}};

  //   //   Model::Networkproblem::Gas::Flowboundarynode  g0("gasnode0", bd_gas0,
  ...
  //   //   gp0.evaluate(rootvalues, last_time, new_time, last_state, new_state -
  //   //   h); N1.evaluate(rootvalues, last_time, new_time, last_state, new_state
  //   //   - h); g0.evaluate(rootvalues, last_time, new_time, last_state,
  //   //   new_state - h);

  //   //   gp0.evaluate(rootvaluesh, last_time, new_time, last_state, new_state +
  //   //   h); N1.evaluate(rootvaluesh, last_time, new_time, last_state, new_state
