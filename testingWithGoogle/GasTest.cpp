...
template <typename T>
nlohmann::json
make_value_json(std::string id, std::string key,
  ...
  TEST_F(GasTEST, Pipe_evaluate_state_derivative) {
   ...
   int number_of_variables = netprob->set_indices(0);
      ...
      netprob->set_initial_values(new_state, net_initial);
            ...
            // Eigen::VectorXd rootvaluesm0(number_of_variables);
            // Eigen::VectorXd rootvalues1(number_of_variables);
            // Eigen::VectorXd rootvalues2(number_of_variables);
            // Eigen::VectorXd rootvalues3(number_of_variables);

            // double epsilon = pow(DBL_EPSILON,1.0/3.0);

        //   //   // I'm reasonably sure that is is ok

        //   //   // std::cout <<RED<< __FILE__ <<":"<<__LINE__ <<": This is a
        //   hack!
        //   //   Maybe its ok, to use twice the squareroot but check this!" <<
        //   //   RESET<<std::endl;

        //   //   std::cout << RED << __FILE__ << ":" << __LINE__
        //   //             << ": Be aware that here twice the squareroot of
        //   machine
        ...
        //   //   ((rootvalues1-rootvalues)/epsilon).segment<2>(1);
        //   Eigen::VectorXd diff2
        ...
        //   //       double max0 = (diff0 -
        //   analytical0).lpNorm<Eigen::Infinity>();
        //   //       double max1 = (diff1 -
        //   analytical1).lpNorm<Eigen::Infinity>();
        //   //       double max2 = (diff2 -
        //   analytical2).lpNorm<Eigen::Infinity>();
        //   //       double max3 = (diff3 -
        //   analytical3).lpNorm<Eigen::Infinity>();
