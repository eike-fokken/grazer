...
namespace Model::Networkproblem::Gas {
  ...
  public:
    ...
    /// Shortpipes just use pressure and volumetric flow as their variables, so
    /// that this function simply returns the corresponding boundary state.
    Eigen::Vector2d get_boundary_p_qvol_bar(
        int direction,
        Eigen::Ref<Eigen::VectorXd const> const &state) const override;

    /// Because shortpipes use pressure and volumetric flow as their state
