...
namespace Model::Networkproblem::Gas {
  ...
  void Gasnode::evaluate_flow_node_balance(
    ...
    int last_equation_index =
        directed_attached_gas_edges.back().second->give_away_boundary_index(
            last_direction);
  ...
  void Gasnode::evaluate_pressure_node_balance(
      Eigen::Ref<Eigen::VectorXd> rootvalues,
      Eigen::Ref<Eigen::VectorXd const>state,
      double prescribed_p) const {
  ...
  void Gasnode::evaluate_flow_node_derivative(
    ...
    for (auto it = second_iterator; it != last_iterator; ++it) {
      ...
      edge->dboundary_p_qvol_dstate(direction, jacobianhandler, dF_old_dpq_now,
                                    old_equation_index, state);
      edge->dboundary_p_qvol_dstate(direction, jacobianhandler, dF_now_dpq_now,
                                    current_equation_index, state);
      edge->dboundary_p_qvol_dstate(direction, jacobianhandler, dF_last_dpq_now,
                                    last_equation_index, state);
      old_equation_index = current_equation_index;
    }
    // last edge:
    Eigen::RowVector2d dF_old_dpq_last(1.0, 0.0);
    edgelast->dboundary_p_qvol_dstate(dirlast, jacobianhandler, dF_old_dpq_last,
                                      old_equation_index, state);
  ...
  void Gasnode::evaluate_pressure_node_derivative(
      Aux::Matrixhandler *jacobianhandler,
      Eigen::Ref<Eigen::VectorXd const>state) const {
  ...
  int Gasnode::get_number_of_states() const {
