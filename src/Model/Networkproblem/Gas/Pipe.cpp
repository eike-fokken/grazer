...
namespace Model::Networkproblem::Gas {
  ...
  void
  Pipe::evaluate(Eigen::Ref<Eigen::VectorXd> rootvalues, double last_time,
                 double new_time,
    ...
    for (int i = get_equation_start_index(); i != get_equation_after_index();
      ...
      scheme.evaluate_point(rootvalue_segment, last_time, new_time, Delta_x,
  ...
  void Pipe::evaluate_state_derivative(
    ...
    for (int i = get_equation_start_index(); i != get_equation_after_index();
      ...
      Eigen::Matrix2d current_derivative_left =
          scheme.devaluate_point_dleft(last_time, new_time, Delta_x, last_left,
      ...
      jacobianhandler->set_coefficient(i + 1, i - 1,
                                       current_derivative_left(1, 0));
      jacobianhandler->set_coefficient(i + 1, i, current_derivative_left(1, 1));

      Eigen::Matrix2d current_derivative_right =
          scheme.devaluate_point_dright(last_time, new_time, Delta_x, last_left,
                                        last_right, new_left, new_right, bl);

      jacobianhandler->set_coefficient(i, i + 1,
                                       current_derivative_right(0, 0));
      jacobianhandler->set_coefficient(i, i + 2,
                                       current_derivative_right(0, 1));
      jacobianhandler->set_coefficient(i + 1, i + 1,
                                       current_derivative_right(1, 0));
      jacobianhandler->set_coefficient(i + 1, i + 2,
                                       current_derivative_right(1, 1));
    }
  }
  ...
  void Pipe::print_to_files(std::filesystem::path const &output_directory) {
    ...
    std::filesystem::path shortpipe_output_pressure(output_directory /
                                                    pressure_file_name);
    std::filesystem::path shortpipe_output_flow(output_directory /
                                                flow_file_name);
  ...
  void Pipe::save_values(double time,
  ...
  void Pipe::set_initial_values(Eigen::Ref<Eigen::VectorXd> new_state,
    ...
    for (int i = 0; i != number_of_points; ++i) {
      try {
        ...
        std::cout << "requested - length: " << (i * Delta_x - length)
                  << std::endl;
        throw;
      }
    }
