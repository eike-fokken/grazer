...
namespace Model {
  ...
  Problem::Problem(nlohmann::json problem_data,
  ...
  void
  Problem::evaluate(Eigen::Ref<Eigen::VectorXd> rootvalues, double last_time,
                    double new_time,
  ...
  void Problem::save_values(double time,
  ...
  void Problem::set_initial_values(Eigen::Ref<Eigen::VectorXd> new_state,
    ...
    for (auto &subproblem : subproblems) {
      ...
      aux_json::replace_entry_with_json_from_file(subproblem_initial_json,
                                                  "initial_json");
      subproblem->set_initial_values(new_state,
