...
namespace Model::Balancelaw {
  ...
  Eigen::Matrix4d const Isothermaleulerequation::coeff_helper_matrix
      = set_coeff_helper_matrix();

  Eigen::Vector4d
  Isothermaleulerequation::get_coefficients(double diameter, double roughness) {
  ...
  double Isothermaleulerequation::lambda_non_laminar(
      double Re, double diameter, double roughness) {
  ...
  double Isothermaleulerequation::dlambda_non_laminar_dRe(
      double Re, double diameter, double roughness) {
    ...
    if (Re < laminar_border - Aux::EPSILON) {
      gthrow(
          {"This function is only valid for Reynolds numbers over and at 2000!",
           " For other values, manually insert "
           "\"-64/(Reynolds(q)*Reynolds(q))\" instead"});
    }
  ...
  double Isothermaleulerequation::Swamee_Jain(
    ...
    if (Re < turbulent_border - Aux::EPSILON) {
      gthrow(
          {"This function must not be called for Reynolds numbers smaller than "
           "4000!"});
    }
  ...
  double Isothermaleulerequation::dSwamee_Jain_dRe(
      double Re, double diameter, double roughness) {
    if (Re < turbulent_border - Aux::EPSILON) {
      gthrow(
          {"This function must not be called for Reynolds numbers smaller "
           "than 4000!"});
    }
  ...
  double Isothermaleulerequation::exact_turbulent_lambda(
      double Re, double diameter, double roughness) {
    if (Re < turbulent_border - Aux::EPSILON) {
      gthrow(
          {"This function must not be called for Reynolds numbers smaller "
