...
namespace Aux::unit {
  ...
  template <typename T>
  const std::tuple<std::string, T>
  parse_unit(std::string const &unit,
    ...
    for (auto const &[str_unit, val] : unit_map) {
      ...
      if (unit_size >= symb_size and
          unit.compare(unit_size - symb_size, symb_size, str_unit) ==
              0) { // match
  ...
  template <typename T>
  double parse_to_si(json const &unit_json,
                     std::map<std::string, T> const &unit_map) {
