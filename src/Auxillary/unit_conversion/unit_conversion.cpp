...
namespace Aux::unit {
  ...
  // the entries
  const std::map<std::string, double> length_units{{"m", 1.0},
                                                   {"in", 0.0254},
                                                   {"ft", 0.3048},
                                                   {"yd", 0.9144},
                                                   {"mi", 1609.344}};
  ...
  const std::map<std::string, double> power_units{{"W", 1.0},
                                                  {"PS", 735.49875}};
  ...
  double parse_prefix(std::string const &prefix,
    ...
    if (last_space != std::string::npos) {
      ...
      if (remaining != last_space) {
        throw std::runtime_error("Multiple spaces in unit not allowed. Format "
                                 "should be [(number) unit]");
      }
  ...
  template double
  parse_to_si<conversion>(json const &unit_json,
                          std::map<std::string, conversion> const &unit_map);
  template double
  parse_to_si<double>(json const &unit_json,
