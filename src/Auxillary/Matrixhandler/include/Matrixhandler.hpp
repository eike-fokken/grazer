...
namespace Aux {
   ...
   *
   */
  class Eigen_handler_base : public Matrixhandler {
  public:
  ...
  protected:
    Eigen_handler_base(Eigen::SparseMatrix<double> *_matrix) :
        matrix(_matrix){};

    Eigen::SparseMatrix<double> *matrix;
  };

  /** \brief The Triplethandler variety gathers the coefficients in triplets
   * and later builds the matrix from the triplets.
   */
  class Triplethandler final : public Eigen_handler_base {

  public:
    Triplethandler(Eigen::SparseMatrix<double> *_matrix) :
        Eigen_handler_base(_matrix){};
   ...
   * contains no state.
   */
  class Coeffrefhandler final : public Eigen_handler_base {
  ...
  /** \brief The Ipopt_handler variety gathers the coefficients in three vectors
   * as required by Ipopt.
   */
  class Ipopt_handler : public Matrixhandler {
